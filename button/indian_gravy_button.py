...
def display_indian_gravy_button(ref, session_state):
    egg_items = fetch_indian_gravy_items(ref)

    if egg_items:
        if st.button("indian gravy", use_container_width=200):
            session_state['button_state_indian gravy'] = not session_state['button_state_indian gravy']

    if session_state['button_state_indian gravy']:
        with st.container():
