...
from firebase_data import fetch_Soft_Drinks_items
...
dref = db.reference('/')
def display_soft_drink_button(table_number, ref, session_state):
    egg_items = fetch_Soft_Drinks_items(ref)

    if egg_items:
        if st.button("Soft Drinks", use_container_width=200):
            session_state['button_state_Soft Drinks'] = not session_state['button_state_Soft Drinks']

    if session_state['button_state_Soft Drinks']:
        with st.container():
