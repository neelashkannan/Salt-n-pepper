...
from firebase_data import fetch_egg_items
...
dref = db.reference('/')
def display_egg_button(table_number, ref, session_state):
    egg_items = fetch_egg_items(ref)

    if egg_items:
        if st.button("egg :egg:", use_container_width=200):
            session_state['button_state_egg'] = not session_state['button_state_egg']

    if session_state['button_state_egg']:
        with st.container():
