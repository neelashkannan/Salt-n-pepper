...
def display_rice_noodles_button(ref, session_state):
    egg_items = fetch_rice_noodles_items(ref)

    if egg_items:
        if st.button("rice /noodles", use_container_width=200):
            session_state['button_state_Rice / Noodles'] = not session_state['button_state_Rice / Noodles']

    if session_state['button_state_Rice / Noodles']:
        with st.container():
